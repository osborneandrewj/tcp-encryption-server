...
int main(int argc, char *argv[]) {
  ...
  // Handshake:
  // Only allow connection with dec_server (not enc_server)
  buffer[0] = '*';
  buffer[1] = 'D';
  int verify_sent = send(socketFD, buffer, sizeof(char)*2, 0);
  if (verify_sent < 0) error("CLIENT: ERROR verifying (send)");
  int verify_read = recv(socketFD, buffer, sizeof(char), 0);
  if (verify_read < 1) error("CLIENT: ERROR verifying (read)");
  if (buffer[0] != 68) {
  ...
  // Read 1 char from ciphertext and 1 char from mykey. These are 
  // combined into a buffer (two_bit_buf) and sent to the dec_server
  // Expected return is 1 char of decrypted data which is printed
  // to stdout
  while (fread(two_bit_buf, sizeof(char) * 1, 1, fp_pt) == 1) {
