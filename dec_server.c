// By Andrew Osborne 16 March 2023
  ...
  for (;;) {
    ...
    switch (fork()) {
      ...
      case 0:
        ...
        // First, verify that the client is dec_client
        // Refuse any connection other than this client
        int verify_request = recv(connectionSocket, buffer, 2, 0);
        if (verify_request < 0) error("ERROR verifying client (request)");
        if (buffer[1] == 68) {
          buffer[0] = 'D';
          int verify_sent = send(connectionSocket, &buffer, sizeof(char), 0);
          if (verify_sent < 0) error("ERROR verifying client (send)");
        } else {
          exit(EXIT_FAILURE);
        }
        
        while ((num_read = read(connectionSocket, buffer, 2)) > 0) {
          // Get ciphertext bye and subtract 65 from the ASCII value to index
          // inot our alpha[] array
          a = buffer[0] - 65;
          // If value is a space then index it to alpha[26]
          if (buffer[0] == 32) {
            a = 26;
          }
          // Get key byte
          ...
          if (charsWritten != 1) error("ERROR sending data to client");
        }
